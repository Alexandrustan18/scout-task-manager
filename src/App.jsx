...
function TasksPage({ fProps, grouped, filtered, user, team, onEdit, onView, onDel, onDup, onChgSt, isMob, timers, getTS, togTimer, bulkMode, selectedTasks, toggleSel }) {
  ...
  var statusGroups = useMemo(function() {
    var groups = {};
    // Sort: admin-created tasks first within each group
    var sortAdminFirst = function(arr) { return arr.sort(function(a, b) { var aAdmin = a.createdBy === "admin" ? 0 : 1; var bAdmin = b.createdBy === "admin" ? 0 : 1; return aAdmin - bAdmin; }); };
...
function TargetsPage({ targets, setTargets, team, tasks, timers, canEdit, visUsers, taskTypes, departments }) {
  ...
  return <div style={{ maxWidth: 800 }}>{canEdit && <div style={{ marginBottom: 16 }}><button style={S.primBtn} onClick={function() { setShowForm(!showForm); }}><Ic d={Icons.plus} size={14} color="#fff" /> Target nou</button></div>}{showForm && <Card style={{ marginBottom: 16 }}><div style={S.fRow}><div style={S.fCol}><label style={S.label}>User</label><select style={S.fSelF} value={form.userId} onChange={function(e) { setForm(Object.assign({}, form, { userId: e.target.value })); }}><option value="">--</option>{visUsers.filter(function(u) { return u !== "admin"; }).map(function(u) { return <option key={u} value={u}>{(team[u] || {}).name}</option>; })}</select></div><div style={S.fCol}><label style={S.label}>Metric (dropdown)</label><select style={S.fSelF} value={form.metric} onChange={function(e) { setForm(Object.assign({}, form, { metric: e.target.value })); }}>{metricOptions.map(function(o) { return <option key={o.id} value={o.id}>{o.l}</option>; })}</select></div></div><div style={S.fRow}><div style={S.fCol}><label style={S.label}>Target zilnic</label><input style={S.input} type="number" value={form.target} onChange={function(e) { setForm(Object.assign({}, form, { target: parseInt(e.target.value) || 0 })); }} /></div><div style={S.fCol}><label style={S.label}>Zile/sapt</label><input style={S.input} type="number" min="1" max="7" value={form.daysPerWeek} onChange={function(e) { setForm(Object.assign({}, form, { daysPerWeek: parseInt(e.target.value) || 5 })); }} /></div></div><div style={{ marginTop: 12, display: "flex", gap: 8 }}><button style={S.primBtn} onClick={save}>Salveaza</button><button style={S.cancelBtn} onClick={function() { setShowForm(false); }}>Anuleaza</button></div></Card>}{targets.map(function(tgt) {
    ...
    var todayDone = tasks.filter(function(t) {
      if (t.assignee !== tgt.userId) return false; if (t.status !== "Done") return false;
    ...
    // Istoric pe ultimele 14 zile
    var histDays = []; var totalExpected = 0; var totalActual = 0;
    for (var d = 13; d >= 0; d--) { var dt = new Date(); dt.setDate(dt.getDate() - d); var dStr = ds(dt);
      var dayDone = tasks.filter(function(t2) { if (t2.assignee !== tgt.userId || t2.status !== "Done" || !t2.updatedAt || ds(t2.updatedAt) !== dStr) return false; if (tgt.metric === "all") return true; if (tgt.metric.startsWith("type:") && t2.taskType === tgt.metric.replace("type:", "")) return true; if (tgt.metric.startsWith("dept:") && t2.department === tgt.metric.replace("dept:", "")) return true; if (tgt.metric.startsWith("plat:") && t2.platform === tgt.metric.replace("plat:", "")) return true; return false; }).length;
      var dow = dt.getDay(); var isWorkday = dow >= 1 && dow <= (tgt.daysPerWeek >= 6 ? 6 : tgt.daysPerWeek >= 7 ? 0 : 5);
    ...
    return <Card key={tgt.id} style={{ marginBottom: 16, borderLeft: "3px solid " + (pct >= 100 ? GR : pct >= 50 ? "#D97706" : "#DC2626") }}>
      ...
      {canEdit && <button style={Object.assign({}, S.chip, { fontSize: 10 })} onClick={function() { var newVal = prompt("Target zilnic nou:", tgt.target); if (newVal && parseInt(newVal) > 0) { setTargets(function(p) { return p.map(function(x) { return x.id === tgt.id ? Object.assign({}, x, { target: parseInt(newVal) }) : x; }); }); } }}>Edit</button>}
      ...
      <div style={{ marginTop: 10, padding: 10, background: totalDeficit > 0 ? "#FEF2F2" : "#F0FDF4", borderRadius: 8 }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, fontWeight: 600 }}><span>Ultimele 14 zile</span><span style={{ color: totalDeficit > 0 ? "#DC2626" : GR }}>Asteptat: {totalExpected} | Facut: {totalActual} | {totalDeficit > 0 ? "Deficit: " + totalDeficit : "Surplus: +" + Math.abs(totalDeficit)}</span></div>
      </div>
      {/* Daily history table */}
      <div style={{ marginTop: 8, maxHeight: 200, overflowY: "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 60px 60px 60px", gap: 2, fontSize: 11 }}>
          <div style={{ fontWeight: 600, color: "#94A3B8", padding: 4 }}>Zi</div><div style={{ fontWeight: 600, color: "#94A3B8", padding: 4, textAlign: "center" }}>Target</div><div style={{ fontWeight: 600, color: "#94A3B8", padding: 4, textAlign: "center" }}>Facut</div><div style={{ fontWeight: 600, color: "#94A3B8", padding: 4, textAlign: "center" }}>+/-</div>
          {histDays.map(function(h) { var isToday2 = h.date === TD; return <><div key={h.date + "l"} style={{ padding: "3px 4px", background: isToday2 ? GR + "12" : "#F8FAFC", fontWeight: isToday2 ? 700 : 400, borderRadius: 3 }}>{h.label}{isToday2 ? " (azi)" : ""}</div><div key={h.date + "e"} style={{ padding: "3px 4px", textAlign: "center", background: "#F8FAFC", borderRadius: 3 }}>{h.expected}</div><div key={h.date + "d"} style={{ padding: "3px 4px", textAlign: "center", background: "#F8FAFC", borderRadius: 3, fontWeight: 600, color: h.done >= h.expected ? GR : "#DC2626" }}>{h.done}</div><div key={h.date + "df"} style={{ padding: "3px 4px", textAlign: "center", borderRadius: 3, fontWeight: 600, color: h.deficit > 0 ? "#DC2626" : h.deficit < 0 ? GR : "#94A3B8", background: h.deficit > 0 ? "#FEF2F2" : h.deficit < 0 ? "#F0FDF4" : "#F8FAFC" }}>{h.deficit > 0 ? "-" + h.deficit : h.deficit < 0 ? "+" + Math.abs(h.deficit) : "0"}</div></>; })}
        </div>
      </div>
    </Card>;
...
function ViewTaskModal({ task, team, user, tasks, setTasks, timers, getTS, togTimer, products, onClose, onEdit, statusHistory }) {
  ...
  return <div style={S.modalOv} onClick={onClose}><div style={Object.assign({}, S.modalBox, { maxWidth: 640 })} onClick={function(e) { e.stopPropagation(); }}>
    ...
    {/* Replace product button - only for Dana */}
    {user === "dana" && t.assignee === user && t.status !== "Done" && <div style={{ marginBottom: 12 }}>
