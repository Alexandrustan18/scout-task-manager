      ...
      return <Card key={d.key} style={{ cursor: "pointer" }}>
        <div onClick={function() { onClickUser(d.key); }}>
          ...
          <div style={{ marginTop: 6, borderTop: "1px solid #F1F5F9", paddingTop: 8 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: "#475569", marginBottom: 4 }}>Atribuite ({d.assignedTasks.length}) | In lucru ({d.workingTasks.length}) | Done ({d.rangeDone})</div>
            {d.workingTasks.slice(0, 4).map(function(t) { return <div key={t.id} style={{ fontSize: 11, padding: "2px 8px", marginBottom: 2, borderRadius: 4, background: SBG["In Progress"], display: "flex", alignItems: "center", gap: 4 }}><span style={{ width: 5, height: 5, borderRadius: "50%", background: SC["In Progress"] }} /><span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.title}</span>{t.shop && <span style={{ fontSize: 9, color: GR }}>{t.shop}</span>}</div>; })}
